...
// "יעד" = כל activity_id שמופיע בלפחות אחת מ-3 הטבלאות העצמאיות הקיימות (favorites/
// planned_activities/visited_activities) - בדיוק כמו TABS ב-app/profile.js ("❤️ הפעילויות
// שלי"), רק מורחב לעמוד מלא. אין מודל "סטטוס יחיד" חדש - טאב = חברות בטבלה המתאימה, ופעילות
// יכולה להופיע בכמה טאבים בו-זמנית (למשל גם שמורה וגם היינו כאן).
const TABS = [
  { key: 'favorites', label: '❤️ שמורים', table: 'favorites' },
...
export default function DestinationsScreen() {
  ...
  const [records, setRecords] = useState(new Map()); // activity_id -> {activity, inFavorites, inPlanned, inVisited, target_date, target_label}
  const [notesByActivity, setNotesByActivity] = useState(new Map());
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [favRes, planRes, visitRes, notes] = await Promise.all([
      ...
      const noteMap = new Map(notes.map((n) => [n.activity_id, n.note]));
      setNotesByActivity(noteMap);
    } catch (err) {
      showNotice(`שגיאה בטעינת היעדים: ${err.message}`);
    }
    setLoading(false);
  }, []);
  ...
  const tabItems = useMemo(() => {
    ...
    const key = tab.key === 'favorites' ? 'inFavorites' : tab.key === 'planned' ? 'inPlanned' : 'inVisited';
    return Array.from(records.values()).filter((r) => r[key]);
  }, [records, activeTab]);
  ...
  const removeFromTab = async (activityId) => {
    ...
    const tab = TABS.find((t) => t.key === activeTab);
    try {
      if (tab.key === 'favorites') await toggleFavorite(session.user.id, activityId, false);
      else if (tab.key === 'planned') await togglePlanned(session.user.id, activityId, false);
  ...
  const openNoteModal = (activityId) => {
    setNoteModalTarget(activityId);
    setNoteModalDraft(notesByActivity.get(activityId) || '');
    setActionMenuFor(null);
  };
  ...
  const saveNoteModal = async () => {
    ...
    setSavingNote(true);
    try {
      await savePersonalNote(session.user.id, noteModalTarget, noteModalDraft);
      const trimmed = noteModalDraft.trim();
      setNotesByActivity((prev) => {
        const next = new Map(prev);
        if (trimmed) next.set(noteModalTarget, trimmed); else next.delete(noteModalTarget);
        return next;
      });
    ...
    <View style={styles.screen}>
      <View style={styles.content}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <Text style={styles.title}>📍 היעדים שלי</Text>
          <Text style={styles.subtitle}>כל המקומות שאתם רוצים לגלות עם הילדים</Text>
              ...
              return (
                <View key={a.id} style={styles.card}>
                  <View style={styles.cardTop}>
                    ...
                    <Pressable style={styles.moreBtn} onPress={() => setActionMenuFor(actionMenuFor === a.id ? null : a.id)}>
                  ...
                  {isPastDue && (
                    <View style={styles.pastDueBox}>
                      ...
                      <View style={styles.pastDueActions}>
                        <Pressable onPress={() => openDateModal(a.id, rec)}><Text style={styles.pastDueLink}>עדכן תאריך</Text></Pressable>
                        <Pressable onPress={() => markVisited(a.id, true)}><Text style={styles.pastDueLink}>סמן כהושלם</Text></Pressable>
                      </View>
                    </View>
                  )}

                  {note ? (
                    <Pressable onPress={() => openNoteModal(a.id)}>
                  ...
                  <View style={styles.actionsRow}>
                    <Pressable
                      style={styles.navBtn}
                      onPress={() => openNavigationTo({ title: a.name, locationName: a.location?.name, city: cityLabel })}
                    >
                  ...
                  {actionMenuFor === a.id && (
                    <View style={styles.actionMenu}>
                      <Pressable style={styles.actionMenuItem} onPress={() => openDateModal(a.id, rec)}>
                        <Text style={styles.actionMenuText}>📅 {rec.target_date || rec.target_label ? 'שנה תאריך' : 'הוסף תאריך'}</Text>
                      </Pressable>
                      <Pressable style={styles.actionMenuItem} onPress={() => openNoteModal(a.id)}>
                        <Text style={styles.actionMenuText}>📝 {note ? 'ערוך הערה' : 'הוסף הערה'}</Text>
                      </Pressable>
                      <Pressable style={styles.actionMenuItem} onPress={() => markVisited(a.id, !rec.inVisited)}>
                        <Text style={styles.actionMenuText}>{rec.inVisited ? '↩️ בטל "היינו פה"' : '✅ היינו פה'}</Text>
                      </Pressable>
                      <Pressable style={styles.actionMenuItem} onPress={() => removeFromTab(a.id)}>
                        <Text style={[styles.actionMenuText, styles.actionMenuDanger]}>❤️ הסר מהיעדים</Text>
                      </Pressable>
                    </View>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  tabsRow: { flexDirection: 'row-reverse', gap: 8, marginBottom: 14 },
  ...
  moreBtn: { padding: 4 },
  moreBtnText: { fontFamily: fonts.bold, fontSize: 18, color: colors.textMuted },
  ...
  actionsRow: { flexDirection: 'row-reverse', marginTop: 10 },
  ...
  actionMenu: { marginTop: 10, borderTopWidth: 1, borderTopColor: colors.borderLight, paddingTop: 8, gap: 2 },
  actionMenuItem: { paddingVertical: 9 },
  actionMenuText: { fontFamily: fonts.semiBold, fontSize: 13, color: colors.textPrimary, textAlign: 'right' },
  actionMenuDanger: { color: colors.danger },
