...
<div class="wrap">
  ...
  <h2>מה באמת השתפר (information gain, כל הריצות)</h2><div id="gain"></div>
  <h2>חוב לפי מקור (משוב ל-THE MONSTER)</h2><div id="sourceDebt"></div>
  <h2>מקרים</h2>
  <div class="toolbar">
...
async function loadSummary() {
  ...
  $('tiles').innerHTML = [['פתוחים (backlog)', s.open], ['לטיפול עכשיו', s.due], ['ממתינים לניסיון הבא', s.awaiting], ['נפתרו (עם שיפור)', s.resolved - (s.resolvedNoGain || 0)], ['נפתרו (בלי שינוי)', s.resolvedNoGain || 0], ['בארכיון', s.archived], ['תקועים מעבר לחלון', s.stuck], ['חוב היסטורי / חדש', (s.historicalDebt || 0) + ' / ' + (s.newDebt || 0)], ['ריצה אחרונה', s.lastRun ? fmt(s.lastRun.finished_at || s.lastRun.started_at) : '-']].map(([l, v]) => '<div class="tile"><b>' + esc(v) + '</b><span>' + esc(l) + '</span></div>').join('');
  ...
  $('byReason').innerHTML = '<table><tr><th>סיבה</th><th>מקרים</th></tr>' + (Object.entries(s.byArchiveReason).map(([k, v]) => '<tr><td>' + esc(k) + '</td><td>' + v + '</td></tr>').join('') || '<tr><td colspan="2" class="muted">אין</td></tr>') + '</table>';
  const GAIN_LABELS = { addressesAdded: 'כתובות', streetAddressesAdded: 'כתובות ברמת רחוב', coordsAdded: 'קואורדינטות', coordsImproved: 'קואורדינטות שהוחלפו (מרכז עיר → מאומת)', venuesLinked: 'קישור למקום קנוני', venueRowsEnriched: 'מקומות קנוניים שהועשרו', venuesCreated: 'מקומות קנוניים שנוצרו', imagesAdded: 'תמונות', brokenImagesReplaced: 'תמונות שבורות שהוחלפו', schedulesAdded: 'לוחות זמנים', regionsAdded: 'אזורים', metadataFieldsAdded: 'שדות מטא-דאטה', incomingPromoted: 'מועמדים שפורסמו', existingEnriched: 'פעילויות קיימות שהועשרו', duplicatesMerged: 'כפילויות שמוזגו (לא פורסמו)' };
  const eff = s.effects || {};
  $('gain').innerHTML = '<table><tr><th>שיפור</th><th>כמות</th></tr>' + Object.entries(GAIN_LABELS).map(([k, l]) => '<tr><td>' + esc(l) + '</td><td>' + (s.gain && s.gain[k] || 0) + '</td></tr>').join('') + '<tr><td><b>אשכולות מקום שנפתרו</b></td><td>' + (eff.venueClustersResolved || 0) + '</td></tr><tr><td><b>מקרים שנפתרו בזכות אשכול</b></td><td>' + (eff.resolvedViaCluster || 0) + '</td></tr><tr><td><b>מקרים שנפתרו מידע קנוני על מקום</b></td><td>' + (eff.resolvedViaVenue || 0) + '</td></tr><tr><td><b>חיפושים חיצוניים שנחסכו</b></td><td>' + (eff.externalLookupsAvoided || 0) + '</td></tr></table>';
  $('sourceDebt').innerHTML = '<table><tr><th>מקור</th><th>מקרים</th><th>פתוחים</th><th>בעיה דומיננטית</th></tr>' + (s.sourceDebt || []).map((r) => '<tr><td>' + esc(r.source) + '</td><td>' + r.cases + '</td><td>' + r.open + '</td><td>' + esc(r.dominantIssue || '') + '</td></tr>').join('') + '</table>';
...
async function loadCases() {
  ...
  $('rows').innerHTML = items.map((c) => '<tr><td>' + esc(c.subject_kind) + '</td><td>' + esc(c.issue) + '</td><td><code>' + esc(c.subject_id.slice(0, 8)) + '</code><br><span class="muted">' + esc(c.subject_label || '') + '</span></td><td>' + c.priority + '</td><td>' + c.attempts + '</td><td>' + fmt(c.last_attempt_at) + '</td><td>' + (c.status === 'open' ? fmt(c.next_attempt_at) : '-') + '</td><td>' + esc((c.methods_tried || []).join(', ')) + '</td><td>' + esc(c.status) + (c.archive_reason ? '<br><b>' + esc(c.archive_reason) + '</b>' : '') + (c.resolution && c.resolution.next_strategy ? '<br><span class="muted">הבא: ' + esc(c.resolution.next_strategy) + '</span>' : '') + '</td><td class="muted">' + esc(c.status === 'archived' && c.resolution && c.resolution.explanation ? 'חסר: ' + c.resolution.explanation.missing + ' | נוסה: ' + (c.resolution.explanation.methods_tried || []).join(',') + ' | לא זמין: ' + (c.resolution.explanation.methods_unavailable || []).map((u) => u.stage).join(',') + ' | למה לא מספיק: ' + c.resolution.explanation.why_insufficient + ' | ייפתח מחדש כאשר: ' + (c.resolution.explanation.reopen_when || []).join('; ') : (c.last_error || (c.resolution ? JSON.stringify(c.resolution).slice(0, 160) : ''))) + '</td></tr>').join('') || '<tr><td colspan="10" class="muted">אין מקרים</td></tr>';
}
