...
async function signInWithProvider(provider) {
  ...
  if (!data?.url) throw new Error(t('auth.errors.oauthIncomplete'));
  ...
  if (result.type !== 'success' || !result.url) {
    throw new Error(t('auth.errors.oauthIncomplete'));
  }
  ...
  if (!hashPart) {
    ...
    throw new Error(errorDescription || t('auth.errors.oauthIncomplete'));
  }
  const hashParams = new URLSearchParams(hashPart);
  const access_token = hashParams.get('access_token');
  const refresh_token = hashParams.get('refresh_token');
  if (!access_token || !refresh_token) {
    throw new Error(hashParams.get('error_description') || t('auth.errors.oauthIncomplete'));
  }
...
export async function completeOAuthRedirect() {
  ...
  if (query.get('error_code') || hashParams0.get('error_code')) {
    ...
    throw new Error(description || t('auth.errors.oauthIncomplete'));
  }
