...
function summarizeSchedules(schedules) {
  if (!schedules || schedules.length === 0) {
    return { availableDays: [], openHours: null, hours: 'שעות לא צוינו' };
  }
  ...
  schedules.forEach((s) => {
    ...
    } else if (s.schedule_type === 'one_time' && s.one_time_date) {
      const d = new Date(s.one_time_date);
  ...
  } else if (schedules[0]?.schedule_type === 'one_time' && schedules[0].one_time_date) {
    hours = new Date(schedules[0].one_time_date).toLocaleDateString('he-IL');
  }

  return { availableDays: Array.from(availableDaysSet), openHours, hours };
}

const SELECT_QUERY = `
  id, name, description, entity_type, min_age, max_age,
  price_type, price_amount, category, duration_minutes, indoor_outdoor,
...
export function mapActivityRow(row) {
  const { availableDays, openHours, hours } = summarizeSchedules(row.activity_schedules);
...
export function formatDistance(activity, deviceCoords) {
  ...
  return activity.city || activity.locationName || 'מיקום לא ידוע';
}
