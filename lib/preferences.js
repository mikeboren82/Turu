...
export async function fetchUserPreferences(userId) {
  const { data, error } = await supabase
    .from('profiles')
    .select('excluded_categories, excluded_cities, default_home_filters, children, saved_locations, notification_prefs, visible_home_filters, benefit_clubs, benefit_clubs_other')
    .eq('id', userId)
    .maybeSingle();
