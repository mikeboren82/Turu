...
export async function fetchUserPreferences(userId) {
  const { data, error } = await supabase
    .from('profiles')
    .select('excluded_categories, excluded_cities, default_home_filters, children, saved_locations, notification_prefs, visible_home_filters, benefit_clubs')
    .eq('id', userId)
    .maybeSingle();
...
// מחדל, לא חוסם שום פונקציונליות אם לא הוגדר.
export async function saveBenefitClubs(userId, clubs) {
  const { error } = await supabase.from('profiles').update({ benefit_clubs: clubs }).eq('id', userId);
  if (error) throw error;
}
