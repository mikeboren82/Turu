...
import { normalizeSearchText, searchTokens } from './searchIntent';
...
// מה בחור כרגע בסעיף "כמה רחוק" של components/LocationQuickPicker.js - מקור-אמת יחיד לתצוגה
// (שורה מסומנת + הערך שמוצג בצידה). 'any' ("ללא הגבלת זמן") שייך לשורת הנסיעה; דקות נחשבות רק
// כש-travelMode הוא 'driving' (travelMinutes נשמר "רדום" במעבר ל-walking/any ולא יוצג כנבחר).
// nationwide/לא-ידוע - אין נקודת-מוצא, אז אין שום בחירת-מרחק פעילה.
export function travelSelection(location) {
  const mode = location?.mode;
  const travelMode = location?.travelMode;
  if (!mode || mode === 'nationwide') return { walking: false, driving: false, minutes: null, noTimeLimit: false };
  return {
    walking: travelMode === 'walking',
    driving: travelMode === 'driving' || travelMode === 'any',
    minutes: travelMode === 'driving' && typeof location.travelMinutes === 'number' ? location.travelMinutes : null,
    noTimeLimit: travelMode === 'any',
  };
}

// "בלי מיקום" - בחירה מפורשת של המשתמש בלי שום הגבלה גאוגרפית (mode:'nationwide', שכבר קיים
// ומטופל ב-matchesLocation/searchOriginCoords/distanceScore כ"אין נקודת-מוצא"). מנקה עיר/אזור/
// קואורדינטות/רדיוס ומשמיט travelMode+travelMinutes (אין נקודת-מוצא, אז אין זמן-נסיעה שיישאר
// "רדום" ויחזור בטעות), כך שהמצב חד-משמעי: לא "לא ידוע" (mode:null - שמסך הבית שואל עליו) ולא
// "מיקום נכשל". שאר ה-filters לא נוגעים כלל. מעבר חזרה לעיר/GPS מקבל את ברירת-המחדל הרגילה.
export function locationWithNoRestriction(location) {
  const { travelMode: _travelMode, travelMinutes: _travelMinutes, ...rest } = location || {};
  return { ...rest, mode: 'nationwide', city: '', region: [], coords: null, addressLabel: null, radiusKm: null };
}
...
// חיפוש טקסט חופשי (filters.q) - בודק התאמה בשם, תיאור, קטגוריה ומיקום, כדי לתפוס גם חיפוש לפי
// שם מקום ולא רק סוג פעילות.
//
// התאמת-טוקנים (2026-09-17): קודם נבדק הביטוי המלא, ואם אינו רצוף - כל טוקן בנפרד חייב להופיע
// איפשהו ב-haystack. זה מה שמאפשר למשתמש לא לדעת את הכותרת המדויקת: "שלושת הדובים" / "זהבה
// דובים" / "זהבה ושלושת" כולם מוצאים את "זהבה ושלושת הדובים". ההתאמה לכל טוקן היא substring
// בתוך ה-haystack המנורמל (לא שוויון-טוקנים), כך שקידומות עבריות מחוברות נתפסות מעצמן -
// "דובים" נמצא בתוך "הדובים" - בלי להסיר קידומות באופן עיוור (ראו הערת הנרמול ב-lib/searchIntent.js).
// טוקן בן תו בודד מסונן כרעש. זו הרחבה בלבד: כל מה שהתאים קודם ממשיך להתאים.
function matchesFreeText(activity, query) {
  if (!query || !query.trim()) return true;
  const haystack = normalizeSearchText(
    [activity.title, activity.description, activity.category, activity.locationName, activity.city, activity.region]
      .filter(Boolean)
      .join(' '),
  );
