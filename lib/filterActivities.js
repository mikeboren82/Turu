...
export function rankActivities(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, spontaneousCoords, originCoords) {
  const matched = dedupeNearIdentical(applyFilters(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities));
  return matched
...
function applyFiltersAtRadius(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, originCoords, radiusKm) {
  if (!originCoords) return [];
  const stageFilters = {
    ...filters,
    location: { ...filters.location, mode: 'address', coords: originCoords, radiusKm },
  };
  return applyFilters(activities, stageFilters, deviceCoords, excludedCategories, benefitClubs, excludedCities);
}
...
export function applyFiltersWithSmartRadius(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, originCoords) {
  const stage1 = applyFilters(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities);
  ...
  for (const km of SMART_RADIUS_STAGES_KM) {
    const stageMatches = applyFiltersAtRadius(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, originCoords, km);
    let addedAny = false;
...
export function rankActivitiesWithSmartRadius(activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, spontaneousCoords, originCoords) {
  const { activities: matched, radiusExpanded: effectiveRadiusKm } = applyFiltersWithSmartRadius(
    activities, filters, deviceCoords, excludedCategories, benefitClubs, excludedCities, originCoords
  );
